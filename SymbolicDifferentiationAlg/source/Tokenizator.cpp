...
void tokenize(const std::string& expr, std::vector<Token>& tokens)
{
	...
	Token::TokenType bufferTokenType = Token::VARIABLE; // тип текущего токена
	...
	for (auto& s : expr)
	{
		...
		auto tokenize_Op_Paranth_Sep = [&]() //лямбда-выражение токенизация операций, скобок и разделителей
		{
			if (isOp)
			{
				...
				{
					tokens.push_back({ std::string{s}, Token::OPERATOR, /*Token::RIGHT*/Token::NONE });
				}
				else
				{
					tokens.push_back({ std::string{s}, Token::OPERATOR, /*Token::LEFT*/ Token::getStrOpAssoc(std::string{s}) } );
				}
